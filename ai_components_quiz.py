...
def create_ques_ans(number_of_qn, topic, standard):
    ...
    # template =f"""Prepare {number_of_qn} multiple choice questions on {board} board {classe} ,{subject} subject ,
    # {lesson} on {topic} in {level} levels of blooms taxonomy. generate a python list which contains {number_of_qn}
    # sublists . In each python sublist , first element should be the question. Second , third and fourth elements
    # should be the only 3 options , and fifth element should be the complete correct option to the question exactly
    # as in options .avoid unnecesary text connotations, extra whitespaces and also avoid newlines anywhere ,
    # terminate the lists and strings correctly"""
    ...
    # generate a python list which contains {number_of_qn} sublists . In each python sublist , first element should
    # be the question. Second , third and fourth elements should be the only 3 options , and fifth element should be
    # the complete correct option to the question exactly as in options .avoid unnecesary text connotations,
...
def report(list, score, total):
    ...
    # Create the linear graph (line plot) with labeled sections
    fig.add_trace(
        go.Scatter(x=[0, total], y=[0, 100], mode='lines', line=dict(color='gray', dash='dash'), name='Diagonal Line'))
    fig.add_trace(
        go.Scatter(x=[0.4 * total, 0.4 * total], y=[0, 120], mode='lines', line=dict(color='red', dash='dash'),
                   name='40% Line'))
    fig.add_trace(
        go.Scatter(x=[0.75 * total, 0.75 * total], y=[0, 120], mode='lines', line=dict(color='blue', dash='dash'),
                   name='75% Line'))
    ...
    fig.add_trace(
        go.Scatter(x=[point_x], y=[point_y], mode='markers', marker=dict(color='green', size=10), name='Plotted Point'))
    ...
    # Label the sections
    fig.add_annotation(text='need to go through the lesson again', x=0.15 * total, y=130, showarrow=False,
                       font=dict(color='red'))
    fig.add_annotation(text='revise concepts of wrong answers', x=0.5 * total, y=125, showarrow=False,
                       font=dict(color='blue'))
    fig.add_annotation(text='well done , look at solutions once', x=0.95 * total, y=130, showarrow=False,
